...
public class MeasuresRenderer extends LabelsRenderer {
  ...
  private void renderMeasurement(boolean renderLabel) {
    ...
    switch (count) {
    case 1:
      drawLine(p[0].sX, p[0].sY, p[0].sZ, m.traceX, m.traceY, p[0].sZ, mad);
      break;
    case 2:
  ...
  void renderDistance(String s, Point3fi a, Point3fi b) {
   if ((tickInfo = m.tickInfo) != null) {
      drawLine(a.sX, a.sY, a.sZ, b.sX, b.sY, b.sZ, mad);
      tickA = a;
      tickB = b;
      if (tickAs == null) {
        tickAs = new P3d();
    ...
    int zA = a.sZ - a.sD - 10;
    int zB = b.sZ - b.sD - 10;
    int radius = drawLine(a.sX, a.sY, zA, b.sX, b.sY, zB, mad);
    if (s == null) {
      return;
	}
    if (mad > 0) {
    ...
    if (m.text == null) {
      ...
      drawString(x, y, zLabel, radius, doJustify && (x - a.sX) * (y - a.sY) > 0, false, true, (doJustify ? 0 : Integer.MAX_VALUE), s);
    } else {
      atomPt.ave(a, b);
  ...
  private void renderAngle(String s, Point3fi a, Point3fi b, Point3fi c) {
    ...
    int zC = c.sZ - c.sD - 10;
    int radius = drawLine(a.sX, a.sY, zA, b.sX, b.sY, zB, mad);
    radius += drawLine(b.sX, b.sY, zB, c.sX, c.sY, zC, mad);
    if (s == null) {
      return;
	}
    ...
    if (m.value > 175) {
      if (m.text == null) {
        ...
        g3d.setC(labelColix);
        drawString(b.sX + offset, b.sY - offset, zB, radius, false, false, false, (doJustify ? 0 : Integer.MAX_VALUE), s);
      } else {
        atomPt.setT(b);
        renderLabelOrMeasure(m.text, s);
      }
    ...
    if (m.isTainted()) {
      double radians = MeasureD.computeAngle(p[0], p[1], p[2], vectorT2, vectorT3, false);
    ...
    for (int i = dotCount; --i >= 0;) {
      ...
      if (m.text == null) {
        g3d.setC(labelColix);
        drawString(p3i.x, p3i.y, zLabel, radius, p3i.x < b.sX, false, false, (doJustify ? b.sY : Integer.MAX_VALUE), s);
      } else {
        atomPt.setT(pointT);
        renderLabelOrMeasure(m.text, s);
      }
  ...
  private void renderTorsion(String s, Point3fi a, Point3fi b, Point3fi c, Point3fi d) {
    ...
    int radius = drawLine(a.sX, a.sY, zA, b.sX, b.sY, zB, mad);
    radius += drawLine(b.sX, b.sY, zB, c.sX, c.sY, zC, mad);
    radius += drawLine(c.sX, c.sY, zC, d.sX, d.sY, zD, mad);
    if (s == null) {
      return;
	}
    ...
    if (m.text == null) {
      g3d.setC(labelColix);
      drawString((a.sX + b.sX + c.sX + d.sX) / 4, (a.sY + b.sY + c.sY + d.sY) / 4, zLabel, radius, false, false, false, (doJustify ? 0 : Integer.MAX_VALUE), s);
    } else {
      atomPt.add2(a, b);
      atomPt.add(c);
      atomPt.add(d);
  ...
  private void renderPendingMeasurement() {
    ...
    g3d.setC(labelColix = (renderLabel ? vwr.cm.colixRubberband : count == 2 ? C.MAGENTA : C.GOLD));
  ...
  protected int drawLine(int x1, int y1, int z1, int x2, int y2, int z2, int mad) {
    ...
    int diameter = (int) (mad >= 20 && exportType != GData.EXPORT_CARTESIAN ?  vwr.tm.scaleToScreen((z1 + z2) / 2, mad) : mad);
