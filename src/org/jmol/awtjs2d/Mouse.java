...
public class Mouse implements GenericMouseInterface {
  ...
  public void processTwoPointGesture(double[][][] touches) {
    ...
    double[][] t2 = touches[1];
    double[] t1first = t1[0];
    double[] t1last = t1[t2.length - 1];
    double x1first = t1first[0];
    double x1last = t1last[0]; 
    double dx1 = x1last - x1first;
    double y1first = t1first[1];
    double y1last = t1last[1]; 
    double dy1 = y1last - y1first;
    V3d v1 = V3d.new3(dx1, dy1, 0);
    double d1 = v1.length();
    double[] t2first = t2[0];
    double[] t2last = t2[t2.length - 1];
    double x2first = t2first[0];
    double x2last = t2last[0]; 
    double dx2 = x2last - x2first;
    double y2first = t2first[1];
    double y2last = t2last[1]; 
    double dy2 = y2last - y2first;
    V3d v2 = V3d.new3(dx2, dy2, 0);    
    double d2 = v2.length();
    ...
    if (cos12 > 0.8) {
      ...
      v1 = V3d.new3(x2first - x1first, y2first - y1first, 0);
      v2 = V3d.new3(x2last - x1last, y2last - y1last, 0);
      double dx = v2.length() - v1.length();
