...
//  protected P3 averageAtomPoint;
  ...
  public int getFirstAtomIndexFromAtomNumber(int atomNumber, BS bsVisibleFrames) {
    ...
    for (int i = 0; i < ac; i++) {
      Atom atom = at[i];
      if (!isDeleted(atom) && atom.getAtomNumber() == atomNumber && bsVisibleFrames.get(atom.mi))
        return i;
    }
    return -1;
  }
...
//    return (coord == null || Double.isNaN(coord.x) ? null : coord);
  ...
  protected void fillADa(AtomData atomData, int mode) {
    ...
    for (int i = 0; i < ac; i++) {
      Atom atom = at[i];
      if (isDeleted(atom) || !isMultiModel && atomData.modelIndex >= 0
  ...
  /**
   * get a list of potential H atom positions based on elemental valence and
   * formal charge
   * 
   * @param bs
   * @param nTotal
   * @param vConnect
   * @param flags
   *        [CALC_H_DOALL | CALC_H_JUSTC | CALC_H_HAVEH | CALC_H_QUICK
   * @return array of arrays of points added to specific atoms
   */
  public P3d[][] calculateHydrogens(BS bs, int[] nTotal, Lst<Atom> vConnect,
                                    int flags) {
    ...
    // check for unit cell ONLY if unit cell is showing
    SymmetryInterface sym = vwr.getOperativeSymmetry();
    if (sym != null && vwr.getObjectMad10(StateManager.OBJ_UNITCELL) == 0)
      sym = null;
    P3d ptTemp = (sym == null ? null : new P3d());
    for (int i = bs.nextSetBit(0); i >= 0; i = bs.nextSetBit(i + 1)) {
      ...
      int n = (doAll || ignoreH || allowH ? atom.getCovalentHydrogenCount()
          : 0);
      ...
      if (nBonds == 0) {
        ...
        case 4:
          ...
          pt = P3d.newP(z);
          pt.add(atom);
          hPt = addH(hAtoms[i], hPt, pt, atom, vConnect, sym, ptTemp);
          //$FALL-THROUGH$
        case 3:
          // nitrogen
          z.set(-0.635d, -0.635d, 0.635d);
        ...
        case 2:
          ...
          //                    or RC=C or C=C
          boolean isEne = (hybridization == 2 || atomicNumber == 5
              || nBonds == 1 && targetValence == 4
              || atomicNumber == 7 && isAdjacentSp2(atom));
          getHybridizationAndAxes(i, atomicNumber, z, x,
              (isEne ? "sp2b" : targetValence == 3 ? "sp3c" : "lpa"), false,
              true, isQuick, null);
          pt = P3d.newP(z);
          pt.scaleAdd2(dHX, z, atom);
          hPt = addH(hAtoms[i], hPt, pt, atom, vConnect, sym, ptTemp);
          getHybridizationAndAxes(i, atomicNumber, z, x,
              (isEne ? "sp2c" : targetValence == 3 ? "sp3d" : "lpb"), false,
              true, isQuick, null);
          //     null = hAtoms[i][0];
          pt = P3d.newP(z);
          pt.scaleAdd2(dHX, z, atom);
        ...
        case 1:
          ...
          case 1:
            ...
            if (atomicNumber == 8
                && atom == atom.group.getCarbonylOxygenAtom()) {
              hAtoms[i] = null;
              continue;
            }
                ...
                (hybridization == 2 || atomicNumber == 5
                    ...
                    || atomicNumber == 7
                        && (atom.group.getNitrogenAtom() == atom
                            ...
                            || isAdjacentSp2(atom)) ? "sp2c" : "sp3d"),
                true, false, isQuick, null) != null) {
              pt = P3d.newP(z);
              pt.scaleAdd2(dHX, z, atom);
              hPt = addH(hAtoms[i], hPt, pt, atom, vConnect, sym, ptTemp);
            }
          ...
          case 2:
            // sp2
            getHybridizationAndAxes(i, atomicNumber, z, x,
                (targetValence == 4 ? "sp2c" : "sp2b"), false, false, isQuick,
                null);
            pt = P3d.newP(z);
            pt.scaleAdd2(dHX, z, atom);
            hPt = addH(hAtoms[i], hPt, pt, atom, vConnect, sym, ptTemp);
  ...
  private static int addH(P3d[] hAtoms, int hPt, P3d pt, Atom atom,
                          Lst<Atom> vConnect, SymmetryInterface sym,
                          P3d ptTemp) {
    if (sym != null) {
      ptTemp.setT(pt);
  ...
  public String getHybridizationAndAxes(int atomIndex, int atomicNumber, V3d z,
                                        V3d x, String lcaoTypeRaw,
                                        boolean hybridizationCompatible,
                                        boolean doAlignZ, boolean isQuick,
                                        T3d ref) {

    String lcaoType = (lcaoTypeRaw.length() > 0 && lcaoTypeRaw.charAt(0) == '-'
        ? lcaoTypeRaw.substring(1)
        : lcaoTypeRaw);
    ...
    case 1:
      ...
      if (isSp3) {
        ...
        for (int i = attached[0].getBondCount(); --i >= 0;) {
          if (attached[0].bonds[i].isCovalentNotPartial0()
        ...
        if (pt != 3) {
          ...
          // PI*2/3
          new M3d()
              .setAA(A4d.new4(z.x, z.y, z.z, (pt == 2 ? 1 : -1) * 2.09439507d))
              .rotate(x);
        }
        z.setT(x);
        x.cross(vTemp, z);
      ...
      int na = attached[0].getCovalentBondCount();
      switch (na) {
      case 3:
        // special case, for example R2C=O oxygen
        // look out for iterative calling
        if (ref != attached[0]) {
          getHybridizationAndAxes(attached[0].i, 0, x, vTemp, "pz", false,
              doAlignZ, isQuick, atom);
          vTemp.setT(x);
      ...
      case 2:
        ...
        while (a0 != null && a0.getCovalentBondCount() == 2) {
          ...
          for (int i = 0; i < bonds.length; i++)
            if (bonds[i].isCovalentNotPartial0()) {
  ...
  public BS getAtomBitsMDa(int tokType, Object specInfo, BS bs) {
    ...
    case T.atomno:
      for (int i = ac; --i >= 0;) {
        if (!isDeleted(at[i]) && at[i].getAtomNumber() == iSpec)
          bs.set(i);
      }
      return bs;
    case T.bonded:
      for (int i = ac; --i >= 0;) {
        if (!isDeleted(at[i]) && at[i].getCovalentBondCount() > 0)
          bs.set(i);
      }
      return bs;
    case T.carbohydrate:
    ...
    case T.elemno:
      for (int i = ac; --i >= 0;) {
        if (!isDeleted(at[i]) && at[i].getElementNumber() == iSpec)
          bs.set(i);
      }
      return bs;
    case T.hetero:
      for (int i = ac; --i >= 0;) {
        if (!isDeleted(at[i]) && at[i].isHetero())
          bs.set(i);
      }
      return bs;
    case T.identifier:
    ...
    case T.leadatom:
      for (int i = ac; --i >= 0;) {
        if (!isDeleted(at[i]) && at[i].isLeadAtom())
          bs.set(i);
      }
      return bs;
    case T.polymer:
    ...
    case T.resno:
      ...
      for (int i = ac; --i >= 0;) {
        if (!isDeleted(at[i]) && at[i].getResno() == iSpec)
          bs.set(i);
      }
      return bs;
    case T.solvent:
      ...
      for (int i = ac; --i >= 0;) {
        ...
        if (g >= JC.GROUPID_WATER && g < JC.GROUPID_SOLVENT_MIN) {
          ...
          for (int j = bonds.length; --j >= 0 && n < 3;)
            if (bonds[j].isCovalentNotPartial0()
    ...
    case T.spec_alternate:
      ...
      for (int i = ac; --i >= 0;)
        if (!isDeleted(at[i]) && isAltLoc(at[i].altloc, spec))
          bs.set(i);
      return bs;
    case T.spec_atom:
      ...
      for (int i = ac; --i >= 0;)
        if (!isDeleted(at[i]) && isAtomNameMatch(at[i], atomSpec, allowStar, allowStar))
          bs.set(i);
      return bs;
    case T.spec_chain:
    ...
    case T.spec_resid:
      // could be PDB type file with UNK
      for (int i = ac; --i >= 0;)
        if (!isDeleted(at[i]) && at[i].group.groupID == iSpec)
          bs.set(i);
      return bs;
    case T.spec_seqcode:
    ...
    case T.inscode:
      for (int i = ac; --i >= 0;)
        if (!isDeleted(at[i]) && at[i].group.getInsCode() == iSpec)
          bs.set(i);
      return bs;
    case T.symop:
      for (int i = ac; --i >= 0;)
        if (!isDeleted(at[i]) && at[i].getSymOp() == iSpec)
          bs.set(i);
      return bs;
    }
    ...
    case T.element:
      ...
      for (int i = ac; --i >= 0;)
        if (!isDeleted(at[i]) && bsTemp.get(at[i].getElementNumber()))
          bs.set(i);
      return bs;
    case T.group:
      // within(group
    ...
    case T.site:
      ...
      for (int i = ac; --i >= 0;)
        if (!isDeleted(at[i]) && bsTemp.get(at[i].atomSite))
          bs.set(i);
      return bs;
    }
  ...
  public void getAtomsInFrame(BS bsAtoms) {
    ...
    bsAtoms.clearAll();
    for (int i = ac; --i >= 0;)
      if (!isDeleted(at[i]) && at[i].isVisible(Atom.ATOM_INFRAME))
        bsAtoms.set(i);
  }
  ...
  public BS getVisibleSet(boolean forceNew) {
    ...
    bsVisible.clearAll();
    for (int i = ac; --i >= 0;)
      if (!isDeleted(at[i]) && at[i].checkVisible())
        bsVisible.set(i);
  ...
  public BS getClickableSet(boolean forceNew) {
    ...
    for (int i = ac; --i >= 0;)
      if (!isDeleted(at[i]) && at[i].isClickable())
        bsClickable.set(i);
